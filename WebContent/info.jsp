...
<%!
	public static boolean compareHooby(String[] arr, String item){
		for(String i : arr){
			if(i.equals(item)){
				return true;
			}
...
<body>
    ...
    <form action="Info" name="regform" id="regform" method="post" onsubmit="return sendit()">
        ...
        <p>
        	성별: <label>남자 <input type="radio" name="gender" value="남자" <%if(mem_gender.equals("남자"))out.print("checked"); %>></label> 
        ...
        <p>취미: 
	        <label>등산<input type="checkbox" name="hobby" value="등산" 
	        	<%if(compareHooby(hobbyArr, "등산"))out.print("checked");%>>
	        </label>
	        <label>게임<input type="checkbox" name="hobby" value="게임"
		        <%if(compareHooby(hobbyArr, "게임"))out.print("checked"); %>>
	        </label>
	        <label>영화감상<input type="checkbox" name="hobby" value="영화감상"
		        <%if(compareHooby(hobbyArr, "영화감상"))out.print("checked"); %>>
		    </label>
	        <label>드라이브<input type="checkbox" name="hobby" value="드라이브"
		        <%if(compareHooby(hobbyArr, "드라이브"))out.print("checked");%>>
		    </label>
	        <label>운동<input type="checkbox" name="hobby" value="운동"
		        <%if(compareHooby(hobbyArr, "운동"))out.print("checked"); %>>
		    </label>
	        <label>독서<input type="checkbox" name="hobby" value="독서"
		        <%if(compareHooby(hobbyArr, "독서"))out.print("checked"); %>>
		    </label>
        </p>
        <p>주민등록번호: <input type="text" name="ssn1"  value="<%=mem_ssn1%>"> - <input type="text" name="ssn2"  value="<%=mem_ssn1%>"></p>
        <p>우편번호: <input type="text" id="sample6_postcode" name="zipcode"  value="<%=mem_zipcode%>" maxlength="5"> <button type="button" onclick="sample6_execDaumPostcode()">검색</button></p>
